...
equiSentence = re.compile(r"<p>.*?([Ee]quiv.*?)<\/p>")
equiOnlySentence = re.compile(r".*?([Ee]quiv.*)")
exclOnlySentence = re.compile(r"<p>.*?([Ee]xcl.*?)<\/p>")
...
pgUrl = "http://www.handbook.unsw.edu.au/vbook2016/brCoursesByAtoZ.jsp?StudyLevel=Undergraduate&descr=All"
pgHtml = urllib2.urlopen(pgUrl).read()
subjectCodeWebsite = re.compile(r"http://www.handbook.unsw.edu.au/undergraduate/courses/2016/ECON1203\.html")
subjectCode = re.findall(subjectCodeWebsite, pgHtml)
...
for hc in subjectCode:
	...
	try:
		...
		if courseCode:
			...
			#exclusion = exclOnlySentence.search(courseCode[0]).group(1)
